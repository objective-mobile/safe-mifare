...
class MifareSafeTag(
    ...
    override fun connect() = mfc.connect()
    ...
    override fun readBlock(blockIndex: Int): ByteArray = mfc.readBlock(blockIndex)
    override fun writeBlock(blockIndex: Int, data: ByteArray) = mfc.writeBlock(blockIndex, data)
    override fun close() = mfc.close()
    override fun id(): ByteArray = tag.id
    override fun keys(): List<ByteArray> = listOf(
        MifareClassic.KEY_DEFAULT,
        MifareClassic.KEY_NFC_FORUM,
        MifareClassic.KEY_MIFARE_APPLICATION_DIRECTORY
    )
